...
scene.at('0.0s', ril, ["/s_new", "city1Gate", cityGateId, 0, common.group, "in", 6, "out", 0], "pleaseWorkCity");
...
for (var j = 0; j < bikeOffsets.length; j++) {
  scene.at((bikeOffsets[j]+0.0)+'s', ril, ["/s_new", "city1Bike", s.nextNodeID(), 0, common.group, "out", 6, "bufnum", buffers['330956__nikiforov5000__bicycle-bell'], "pitch", 1.0, "pan", 0, "depth", 0.5, "mul", 0.4], "pleaseWorkCity");
  scene.at((bikeOffsets[j]+6.5)+'s', ril, ["/s_new", "city1Bike", s.nextNodeID(), 0, common.group, "out", 6, "bufnum", buffers['330956__nikiforov5000__bicycle-bell'], "pitch", 1 + (Math.random() / 2), "pan", -1, "depth", 0.5, "mul", 0.2], "pleaseWorkCity");
  scene.at((bikeOffsets[j]+8.5)+'s', ril, ["/s_new", "city1Bike", s.nextNodeID(), 0, common.group, "out", 6, "bufnum", buffers['330956__nikiforov5000__bicycle-bell'], "pitch", 1 - (Math.random() / 2), "pan", 1, "depth", 0.5, "mul", 0.2], "pleaseWorkCity");
...
for (var j = 0; j < scoreOffsets.length; j++) {
  for (var i = 0; i < score.length; i++) {
    scene.at((scoreOffsets[j] + score[i].offset) + 's', ril, ["/s_new", "city1CarHorn", s.nextNodeID(), 0, common.group, "out", 6, "bufnum", buffers['331540__jmpeeples__2000-nissan-maxima-car-horn'], "pitch", score[i].pitch, "pan", score[i].pan, "depth", score[i].depth],  "pleaseWorkCity");
    groupCtr += 1;
  }
}

// airpop goes to 4, which is where our entry bus is
common.airpopUp(20, 3, 20, 0, 4, scene);
