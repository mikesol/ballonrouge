...
scene.at('0.01s', ril, ["/s_new", "verbBus", s.nextNodeID(), 0, group, "in", 4, "out", 0]);

for (var i = 0; i < 6; i++) {
  scene.at((0.1 + (i * 0.5)) + 's', ril, ["/s_new", "klankPitchShift", s.nextNodeID(), 0, group, "out", 4, "bufnum", _.values(mpqq)[0], "shift", i === 0 ? 1.0 : 0.9, "mul", Math.pow(0.95, i * 20)]);
}
...
module.exports = {
  ...
  'SynthDef.new("klankPitchShift",{|out, bufnum, shift, mul = 1.0|Out.ar(out, HPF.ar(PitchShift.ar(Klank.ar(`[[200, 1040, 2481, 3605], nil, [1, 1, 1, 1]], PlayBuf.ar(2,bufnum)), 0.2, shift), 8000) * mul);})',
